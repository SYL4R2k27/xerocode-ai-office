...
BELSI_PROMPT_TEMPLATES: Dict[str, Dict[str, Any]] = {
    "photo_quality": {
        # v1.3.1 (2026-05-10): отсутствие работника в кадре больше НЕ issue.
        # Добавлено has_worker_in_frame; has_helmet теперь bool|null (null если работника нет).
        # Разделены issues (только критичное) и info (нейтральные наблюдения).
        "system": (
...
async def call_with_fallback(
    ...
    if isinstance(payload, AnalyzeImageRequest):
        ...
        # GenerateRequest
        try:
            user_text = user_template_str.format(**(payload.data or {}))
